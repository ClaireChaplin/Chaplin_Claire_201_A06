...
[https://github.com/ClaireChaplin/Chaplin_Claire_201_A06.git](https://github.com/ClaireChaplin/Chaplin_Claire_201_A06.git)
:::
...
print(player_info[[2]][[4]]) # extracting with indexing

print(player_info$LJ$points) # extracting with names
...
player_info$MJ$height <- 78 # adding 'height' element and height to each player list
...
ppg_list <- list() # initializing empty list 

for (player in names(player_info)) { # iterating through players
  points_per_game <- player_info[[player]]$points / player_info[[player]]$games_played # calculating points per game for each player
  ppg_list[[player]] <- points_per_game # adding player's points per game to list
}
...
ppg_rounded <- sapply(ppg_list, function(x) round(x, 2)) # using sapply to produce a vector, using round function with 2 decimals
...
round_fun <- function(x) { #defining function to use in apply function
  if (x < 100) { # values with 2 digits
    return(round(x, -1)) # round to nearest tens
  } else if (x < 1000) { # values with 3 digits
    return(round(x, -2)) # round to nearest hundreds
  } else if (x >= 1000) { # value with 4 digits
    return(round(x, -3)) # round to nearest thousands
  } else { # value with 1 digit
    return(x) 
  }
}

rounded_mat <- apply(mat_rand, MARGIN = 1:2, round_fun) # using apply function with function defined above on matrix
...
avg_temp <- apply(max_temps[,3:ncol(max_temps)], 2, mean) # applying mean function to all temperature columns
...
colorado_data <- max_temps[max_temps$state == "CO", ] # slicing for "CO" rows, all columns

ohio_data <- max_temps[max_temps$state == "OH", ] # slicing for "OH" rows, all columns

virginia_data <- max_temps[max_temps$state == "VA", ] # slicing for "VA" rows, all columns
...
medians <- apply(colorado_data[,3:ncol(colorado_data)], 1, median) # applying median function to all temperature values by row in Colorado data set

names(medians) <- c(colorado_data$year) # assigning year names to median values

max_index <- which.max(medians) # determining the index of the max median
min_index <- which.min(medians) # determining the index of the min median

print(paste0("The maximum median annual temperature of ", medians[max_index], " occurred in ", colorado_data$year[max_index], " and the minimum median annual temperature of ", medians[min_index], " occurred in ", colorado_data$year[min_index], "."))
...
sum_digits <- function(num){ # defining function
  if (num == 0){ # base case
    return(0) 
  }else{ # recursive case
    last_digit <- num %% 10 # determines last digit
    trunc_num <- num %/% 10 # cuts off last digit
    return(last_digit + sum_digits(trunc_num)) # adds last digit to sum, then runs function with truncated number
    }
}
