...
https://github.com/ClaireChaplin/Chaplin_Claire_201_A06.git
...
for (player in names(player_info)) {
  ...
  ppg_list[[player]] <- points_per_game
}
...
```{r}
ppg_rounded <- sapply(ppg_list, function(x) round(x, 2))
...
rounded_mat <- apply(mat_rand, MARGIN = 1:2, round_fun)
print(rounded_mat)
```
...
```{r}
mean_temps_list <- list()

for(month in 3:ncol(max_temps)){
  mean_temp <- mean(max_temps[[month]])
  mean_temps_list[[month]] <- mean_temp
}

mean_temps_df <- data.frame(
  month = c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'),
  mean_temp = unlist(mean_temps_list)
)
...
medians <- apply(colorado_data[,3:ncol(colorado_data)], 1, median)

names(medians) <- c(colorado_data$year)
